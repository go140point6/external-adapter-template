...
contract Consumer is PluginClient, Ownable {
  ...
  //_ids should be the name of your source token from which you want the comparison 
  //_vs_currencies should be the name of your destinaiton token to which you need the comparison
  //_jobID should be tagged in Oracle
  //_oracle should be fulfiled with your plugin node address

  function requestData(address _oracle, string _jobId,string _ids ,string _vs_currencies)
    public
    onlyOwner
    returns (bytes32 requestId)
  {
    ...
    req.add("ids",_ids);
    req.add("vs_currencies",_vs_currencies);
